<script lang="ts" module>
    ...
    function AppRunsOnLegacyTouchDevice(): boolean {
        if (memoized.AppRunsOnLegacyTouchDevice == null) {
            memoized.AppRunsOnLegacyTouchDevice =
                ...
                !MediaQuery("-moz-touch-enabled") &&
                ("ontouchstart" in Window ||
                    (navigator.maxTouchPoints || 0) > 0 ||
                    /touch|android|iphone|ipod|ipad/i.test(
                        navigator.userAgent,
                    ));
        }
    ...
    function updateDevicePointingAccuracy(): void {
        ...
        if (DocumentIsReady()) {
            ...
            if (
                (updatedPointingAccuracy === "none") !==
                    ClassList.contains("noPointer") ||
                (updatedPointingAccuracy === "fine") !==
                    ClassList.contains("finePointer") ||
                (updatedPointingAccuracy === "coarse") !==
                    ClassList.contains("coarsePointer")
            ) {
    ...
    function registerHandler(
        ...
        let EventHandlerRegistry: SDI_EventHandlerRegistry =
            memoized.EventHandlerRegistry; // ref., not copy!
    ...
    function invokeRegisteredHandlers(): void {
        ...
        for (let i = 0, l = EventHandlerRegistry.length; i < l; i++) {
            ...
            try {
                ...
                console.warn(
                    "PointingAccuracy observation function failed with",
                    Signal,
                );
            }
    ...
    class DeviceInfo {
        ...
        static get isLegacyTouchDevice() {
            return AppRunsOnLegacyTouchDevice();
        }

        static get PointingAccuracy() {
        ...
        static get observesPointingAccuracy() {
